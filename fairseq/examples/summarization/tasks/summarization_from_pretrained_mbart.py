...
@register_task('summarization_from_pretrained_mbart')
class SummarizationFromPretrainedMBARTTask(TranslationFromPretrainedBARTTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--doc-lang', required=True, help='document language')
        parser.add_argument('--sum-lang', required=True, help='summary language')
        parser.add_argument('--fixed-decoder', action='store_true', help='fixed decoder during from pretrain')
        parser.add_argument('--trg-prepend', action='store_true', help='add language tag in front of the trg')
        parser.add_argument('--add-task', action='store_true', help='add task ids')
        # fmt: on

    def __init__(self, args, src_dict, tgt_dict):
        super().__init__(args, src_dict, tgt_dict)
        src_dict.add_symbol('[{}]'.format(args.doc_lang))
        tgt_dict.add_symbol('[{}]'.format(args.sum_lang))
        logger.info("bos %d, pad %d, eos %d, unk %d", 
                src_dict.index('<s>'),src_dict.index('<pad>'),
                src_dict.index('</s>'),src_dict.index('<unk>')
                )
        logger.info("src %s: %d; tgt %s:%d", 
                args.doc_lang, src_dict.index('[{}]'.format(args.doc_lang)),
                args.sum_lang, tgt_dict.index('[{}]'.format(args.sum_lang))
                )
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_sumdataset(
            data_path, split, 
            src, self.src_dict, self.args.doc_lang,
            tgt, self.tgt_dict, self.args.sum_lang,
    ...
    def build_model(self, args):
        ...
        if getattr(args, "fixed_decoder", False):
            logger.info("Fix Decoder!")
            model.decoder.requires_grad_(False)
            model.encoder.embed_positions.weight.requires_grad_(True)
            # model.decoder.embed_positions.weight.requires_grad_(True)
            model.decoder.embed_tokens.weight.requires_grad_(False)

        not_require_grad = []
        for name, param in model.named_parameters():
            if param.requires_grad == False:
                not_require_grad.append(name)
        print(not_require_grad)
