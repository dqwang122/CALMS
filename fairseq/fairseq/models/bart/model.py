...
class BARTModel(TransformerModel):
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if self.args.task == 'generation_from_pretrained_mbart' and loaded_dict_size < len(self.encoder.dictionary):
            logger.info(
                "Adding extra dictionary embeddings not found in pretrained model for "\
                "continued pretraining of MBART."
            )
            num_langids_to_add = len(self.encoder.dictionary) - loaded_dict_size
            embed_dim = state_dict['encoder.embed_tokens.weight'].size(1)
            new_lang_embed_to_add = torch.zeros(num_langids_to_add, embed_dim)
            ...
            state_dict['decoder.output_projection.weight'] = state_dict['decoder.embed_tokens.weight'].clone()
